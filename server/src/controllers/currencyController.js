...
module.exports = {
      ...
      // SUCCESS: Push object properties to array and send to client
        ...
        snapshot.forEach(doc => {
          docs.push({
            ...
            price_change_24h: doc.data().price_change_24h,
            price_change_percentage_24h: doc.data().price_change_percentage_24h,
            high_24h: doc.data().high_24h,
            low_24h: doc.data().low_24h,
  ...
  async postCurrency(req, res, next){
    ...
    // (c) Store the currency document query in variable & call ADD method
    try {
      const currencyRef = db.collection('currency');
      const response = await currencyRef.add({
        ...
        price_change_24h: Number(req.body.price_change_24h),
        price_change_percentage_24h: Number(req.body.price_change_percentage_24h),
        high_24h: Number(req.body.high_24h),
        low_24h: Number(req.body.low_24h),
        status: req.body.status,
        description: req.body.description,
        total_volume: Number(req.body.total_volume),
